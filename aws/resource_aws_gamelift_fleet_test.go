...
func TestAccAWSGameliftFleet_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSGameliftFleetBasicConfig(fleetName, buildName, bucketName, zipPath, roleName, policyName),
...
func testAccAWSGameliftFleetBasicConfig(fleetName, buildName, bucketName, zipPath, roleName, policyName string) string {
	return fmt.Sprintf(`
resource "aws_gamelift_fleet" "test" {
  build_id = "${aws_gamelift_build.test.id}"
  ec2_instance_type = "t2.micro"
  name = "%s"
  server_launch_path = "/GomokuServer.exe"
}
...
`, fleetName, buildName, bucketName, zipPath, zipPath, roleName, policyName)
}
